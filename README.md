...
每天自动追踪 arXiv 新论文，按你的研究方向打分，并生成中文论文摘要。项目使用 GitHub Actions 自动抓取论文，用 GitHub Pages 展示网页。默认页面分成“每日新论文”和“顶会精品”两个集合，互不挤占存储上限。
...
会议源会先从 DBLP 获取题录、作者、DBLP 链接以及可用的 DOI/出版社链接。对筛选后的会议论文，系统会再按标题去 arXiv 搜索同题预印本；如果标题能可靠匹配，就回填 arXiv 摘要、PDF 链接和分类，再进入中文翻译流程。没有在 arXiv 找到摘要的会议论文不会默认让模型凭标题猜创新点。
...
| `MAX_CONFERENCE_ARXIV_ENRICHMENTS` | `50` | 每次最多对多少篇会议候选论文按标题查询 arXiv 补摘要 |
| `CONFERENCE_ARXIV_DELAY_SECONDS` | `3` | 会议论文 arXiv 标题查询之间的间隔，避免请求过密 |
| `CONFERENCE_ARXIV_SEARCH_RESULTS` | `5` | 每篇会议论文标题在 arXiv 返回的候选数量 |
